...
Student  harry(975), nancy;
	...
	/*
	 temporary object has no name and is destroyed as the last step 
	 in evaluating the expression that contains its creation point.
	 With this line(nancy = Student(428);) the fllowings happen:
	 1- A temp Student object created using single-argument(int) 
	 constrcutor due to this(Student(428);)
	 2- Using the temp Student object the assignment to nancy is done.
	 3- Temporary object is destroyed using the destrcutor.
	 4- The code flow goes to next line.,
	*/
	nancy = Student(428); //
	nancy.display();
