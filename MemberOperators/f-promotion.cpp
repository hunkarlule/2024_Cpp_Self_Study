...
int main() {
	...
	*/
	...
	/*
	For the same result as the above cast, we may omit the cast 
	operator and defer to the compiler promoting the int value 466 
	to a Student object before assigning the object to nancy:

	The compiler inserts code that creates a temporary Student 
	object using the single-argument constructor. 
	The constructor receives the value 466 and initializes no to 466 
	and ng to 0. Then, the assignment operator copies the temporary 
	object to nancy. Finally, the compiler inserts code that destroys 
	the temporary object removing it from memory.
	*/
    
	// here when compliler sees that we try to assing an int to a Student
	// object. It tries to do its best to make this work. It sees that
	// Stundet class have single-argument(int) constructor. So, it uses it and
	// creates a temp student object out of 466 before doing the assignment.
	nancy = 466; // promotes an int to a Student
	nancy.display();
	cout << endl;
}
