...
const int NG = 20;
/*
Understanding Order - Destruction

Object destruction proceeds in the following order:

Execute the logic of the object's destructor
Deallocate memory for each instance variable in opposite 
order to that listed in the class definition

Member Function Calls
An object's destructor starts executing only after every normal 
member function has completed its execution.

Client code cannot call any member function on an object after 
the object has called its destructor and gone out of scope.
*/
class Student {
	int no;
	float grade[NG];
